...
Data <- as.data.table(read_csv("/Users/AaronBerman/Downloads/ARYearly.csv"))
```
...
We also need to account for extreme values. Replace any extreme value with the farthest non-outlier value. This technique is refered to as winsorizing, which you can read about here: https://www.statisticshowto.datasciencecentral.com/winsorize/. Winsorizing makes our model more robust to outliers. 
...
mlr1 <- lm(NextYearlyRet ~ zroe + zroa + zfcf + zroic + zpe + zpb, by="year", data=Data)
mlr1 %>% tidy()
...
mlr5 <- lm(NextYearlyRet ~ (zroe*zroic + zfcf + zpe + zpb + capGroup), data=Data)
mlr5 %>% tidy()
